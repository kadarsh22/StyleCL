...
def training_loop(
    ...
    common_kwargs = dict(c_dim=training_set.label_dim, img_resolution=training_set.resolution, img_channels=3,
                         actual_data_img_channels=training_set.num_channels)
    ...
    if (resume_pkl is not None) and (rank == 0):
        ...
        with dnnlib.util.open_url('pretrained_models/lsun_church/lsun_church_fid17.pkl') as f:
            resume_data = legacy.load_network_pkl(f)
        for name, module in [('G', G)]:
            misc.copy_params_and_buffers(resume_data[name], module, require_all=False)

        misc.print_differing_tensor(resume_data['G'], G)

    # Print network summary tables.
    if rank == 0:
        x = torch.empty([batch_gpu, G.num_ws, codebook_size], device=device)
        ws = misc.print_module_summary(adaptor, [x])
        c = torch.empty([batch_gpu, G.c_dim], device=device)
        img = misc.print_module_summary(G, [ws])
    ...
    params_list = []
    learnable_g_params = ['skip_strength_flowers','skip_strength_church']
    print("Learnable Parameters : ")
    for param_name, parameter in G.named_parameters():
        try:
            network_type = param_name.split('.')[2]
        except IndexError:
            network_type = 'Not exist'
        if network_type in learnable_g_params:
            print(param_name)
    ...
    # wandb.watch(adaptor, log = 'all', log_freq = 200)
    for phase in phases:
    ...
    if rank == 0:
        ...
        images_gen = []
        for w in w_s:
            image = G.synthesis(w).cpu()
            images_gen.append(image)
        images = torch.cat(images_gen).numpy()
    ...
    while True:
        ...
        if (network_snapshot_ticks is not None) and (done or cur_tick % network_snapshot_ticks == 0):
            ...
            for name, module in [('adaptor', adaptor), ('D', D), ('G', G), ('augment_pipe', augment_pipe)]:
