...
class SynthesisBlock(torch.nn.Module):
    def __init__(self,
                 ...
                 out_channels,  # Number of output channels.
                 actual_data_img_channels,  # the number of image channels of data
                 w_dim,  # Intermediate latent (W) dimensionality.
        ...
        self.actual_data_img_channels = actual_data_img_channels
        ...
        if in_channels != 0 and architecture == 'skip':
            self.skip = Conv2dLayer(in_channels, out_channels, kernel_size=1, bias=False, up=2,
                                    resample_filter=resample_filter, channels_last=self.channels_last)
            self.skip_strength = torch.nn.Parameter(torch.zeros([]))
    ...
    def forward(self, x, img, ws, force_fp32=False, fused_modconv=None, **layer_kwargs):
        ...
        if img is not None and self.actual_data_img_channels == 1:
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(self,
        ...
        for res in self.block_resolutions:
            ...
            block = SynthesisBlock(in_channels, out_channels, actual_data_img_channels, w_dim=w_dim, resolution=res,
...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        self.img_channels = img_channels
        self.actual_data_img_channels = actual_data_img_channels,
        self.synthesis = SynthesisNetwork(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, actual_data_img_channels =actual_data_img_channels,
                                          **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        # self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)

    def forward(self, ws, truncation_psi=1, truncation_cutoff=None, **synthesis_kwargs):
